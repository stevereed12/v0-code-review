    ...
    log.signal === "BUY"
      ...
      : log.signal === "SELL" || log.signal === "FADE"
        ...
        : log.signal === "WATCH"
          ...
          : "#3d4f6b"
  const statusColor =
    log.status === "APPROVED" ? "#00ffaa" : log.status === "PASSED" ? "#3d4f6b" : "#facc15"
  const date = new Date(log.ts)

  return (
    <div
      className="bg-[#0c1020] border border-[#131c2e] rounded p-3 mb-2"
      style={{ borderLeft: `3px solid ${sigColor}` }}
    >
      <div className="flex justify-between items-baseline mb-1.5 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[13px] text-[#d6dff0] font-medium tracking-wide">
            {log.ticker}
          </span>
          <span
      ...
      <div className="text-xs leading-snug text-[#d6dff0] mb-2">{log.play}</div>

      <div className="font-mono text-[9px] text-[#3d4f6b] tracking-wide mb-2.5">
      ...
      <div className="flex gap-1.5 mb-1.5 flex-wrap items-center">
        <span className="font-mono text-[8px] text-[#3d4f6b] tracking-wider pt-1 mr-1">STATUS:</span>
      ...
      {log.status === "APPROVED" && (
        <div className="flex gap-1.5 items-center flex-wrap">
          <span className="font-mono text-[8px] text-[#3d4f6b] tracking-wider pt-1 mr-1">OUTCOME:</span>
          {(["WIN", "EVEN", "LOSS"] as const).map((o) => {
            const oc = o === "WIN" ? "#00ffaa" : o === "LOSS" ? "#f87171" : "#facc15"
            return (
              <button
                key={o}
                ...
                style={
                  log.outcome === o
                    ...
                    : { borderColor: "#1a2540", color: "#3d4f6b" }
                }
              >
                {o}
              </button>
          ...
          <button
            onClick={onDelete}
            className="ml-auto text-[#3d4f6b] hover:text-[#f87171] transition-colors p-1"
          >
